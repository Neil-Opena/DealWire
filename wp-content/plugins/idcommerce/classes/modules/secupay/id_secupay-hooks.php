...
function id_secupay_localization_strings($strings) {
	$strings['pay_with_prepay'] = __('Pay with Prepay', 'memberdeck');
			...
			if (!empty($level)) {
				...
				if (!empty($request_data)) {
					...
					if (!empty($submit_name)) {
						...
						$request_data = array_merge($request_data, $default_data, $sale_data);
	...
	if (isset($_GET['memberdeck_notify'])) {
		...
		switch ($source) {
			case 'secupay':
				...
				switch ($post_params->payment_status) {
					case 'accepted':
						/*
						...
						*/
						...
						if (empty($txn_check)) {
							...
							// time to add order
							$order = new ID_Member_Order(null, $user_id, $get_params->product_id, null, $api_return->data->trans_id, null, 'active', $e_date, $get_params->price);
					...
					default:
						// awaiting other possible status codes
						break;
				}
				break;
