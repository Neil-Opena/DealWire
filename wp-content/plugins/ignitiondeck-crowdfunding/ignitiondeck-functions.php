...
function getProductDefaultSettings() {
	$settings = ID_Project::get_project_defaults();
	if (count($settings) > 0)
		return $settings;
	else
		return -1;
...
function get_product_levels_callback() {
	...
	if (isset($_POST['Project'])) {
		...
		$options = "<option data-price='".number_format($meta_price_1, 2, '.', '')."' value=\"1\">".__('Level', 'ignitiondeck')." 1: ".apply_filters('id_display_currency', apply_filters('id_price_format', absint($meta_price_1), $post_id), absint($meta_price_1), $post_id)."</option>";
