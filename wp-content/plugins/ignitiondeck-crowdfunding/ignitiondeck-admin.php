...
function form_settings(){
    ...
    if(isset($_POST['submit'])){
        ...
        if($_POST['submit'] == __('Save Settings', 'ignitiondeck')){	
        ...
        if($_POST['submit'] == __('Update Settings', 'ignitiondeck')){	

            $sql_update="update ".$wpdb->prefix . "ign_form set form_settings='".$serializedForm."' where id='1'";
            $res = $wpdb->query( $sql_update );
            $message = '<div class="updated fade below-h2" id="message" class="updated"><p>'.__('Form Settings Successfully Updated', 'ignitiondeck').'</p></div>';	
            echo $message;
        }
    }
...
function email_settings() {
	...
	if (isset($_POST['submitEmailSettings'])) {
	    ...
	    $message = '<div class="updated fade below-h2" id="message" class="updated"><p>'.__('Email Settings Saved', 'ignitiondeck').'</p></div>';
	}
...
function paypal_payment_options() {
    ...
    if(isset($_POST['btnSaveAdaptivePayment'])){
        if($_POST['btnSaveAdaptivePayment'] == __('Save Settings', 'ignitiondeck')) {
            ...
            $message = '<div class="updated fade below-h2" id="message" class="updated"><p>'.__('Payment Settings Saved', 'ignitiondeck').'</p></div>';
        }
    }
	...
	if(isset($_POST['btnSavePaymentSettings'])) {
        if($_POST['btnSavePaymentSettings'] == __('Save Settings', 'ignitiondeck')) {
            // check if it is first time data entered
